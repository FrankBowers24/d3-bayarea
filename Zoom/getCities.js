...
var fs = require('fs');
var geom = JSON.parse(fs.readFileSync('Bay_Area_topo.json', 'utf8'));
...
var i = 0;
var numZips = geom.objects.Bay_Area.geometries.length;
var numCities = 0;
console.log("numZips: " + numZips);
for ( ;i < numZips; i++) {
  console.log(geom.objects.Bay_Area.geometries[i].properties.GEOID10);  
  getCity(geom.objects.Bay_Area.geometries[i].properties);
}
  ...
  res.on('data', function (data) {
       ...
       if (jsonObject.city) {
         ...
         if (++numCities == numZips) {
         	fs.writeFileSync('Bay_Area_Cities_topo.json', JSON.stringify(geom));
